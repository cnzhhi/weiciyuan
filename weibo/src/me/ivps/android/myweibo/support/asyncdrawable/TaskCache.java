...
public class TaskCache {
    ...
    private static final ThreadFactory sDownloadThreadFactory = new ThreadFactory() {
        ...
        public Thread newThread(Runnable r) {
            return new Thread(r, "DownloadFutureTask Download #"
                    + mCount.getAndIncrement());
        }
    };
    
    private static final Executor DOWNLOAD_THREAD_POOL_EXECUTOR = new ThreadPoolExecutor(
            4, 4, 1, TimeUnit.SECONDS, new LinkedBlockingDeque<Runnable>(15) {
                @Override
    ...
    public static void removeDownloadTask(String url,
            DownloadFutureTask downloadWorker) {
    ...
    public static boolean waitForPictureDownload(String url,
            ...
            String savedPath, FileLocationMethod method) {
        while (true) {
            DownloadFutureTask downloadFutureTask = TaskCache.downloadTasks
                    .get(url);
            ...
            if (downloadFutureTask == null) {
                ...
                synchronized (backgroundWifiDownloadPicturesWorkLock) {
                    downloadFutureTask = TaskCache.downloadTasks.putIfAbsent(
                            url, newDownloadFutureTask);
                }
                if (downloadFutureTask == null) {
