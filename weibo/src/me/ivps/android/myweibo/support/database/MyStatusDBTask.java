...
public class MyStatusDBTask {
    ...
    public static void add(MessageListBean list, String accountId) {
        ...
        DatabaseUtils.InsertHelper ih = new DatabaseUtils.InsertHelper(
                getWsd(), MyStatusTable.StatusDataTable.TABLE_NAME);
        final int mblogidColumn = ih
                .getColumnIndex(MyStatusTable.StatusDataTable.MBLOGID);
        final int accountidColumn = ih
                .getColumnIndex(MyStatusTable.StatusDataTable.ACCOUNTID);
        final int jsondataColumn = ih
                .getColumnIndex(MyStatusTable.StatusDataTable.JSONDATA);
        try {
    ...
    public static void clear(String accountId) {
        String sql = "delete from " + MyStatusTable.StatusDataTable.TABLE_NAME
                + " where " + MyStatusTable.StatusDataTable.ACCOUNTID + " in "
                + "(" + accountId + ")";
    ...
    public static MyStatusTimeLineData get(String accountId) {
        ...
        String sql = "select * from "
                + MyStatusTable.StatusDataTable.TABLE_NAME + " where "
                + MyStatusTable.StatusDataTable.ACCOUNTID + "  = " + accountId
                + " order by " + MyStatusTable.StatusDataTable.MBLOGID
                + " desc limit 50";
    ...
    private static void updatePosition(TimeLinePosition position,
        ...
        String sql = "select * from " + MyStatusTable.TABLE_NAME + " where "
                + MyStatusTable.ACCOUNTID + "  = " + accountId;
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        if (c.moveToNext()) {
            try {
                ...
                getWsd().update(MyStatusTable.TABLE_NAME, cv,
                        MyStatusTable.ACCOUNTID + "=?", args);
            }
    ...
    private static TimeLinePosition getPosition(String accountId) {
        ...
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(MyStatusTable.TIMELINEDATA));
    ...
    public static void asyncReplace(final MessageListBean data,
