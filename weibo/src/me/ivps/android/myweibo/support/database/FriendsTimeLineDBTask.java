...
public class FriendsTimeLineDBTask {
    ...
    public static void asyncUpdatePosition(final TimeLinePosition position,
        ...
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                FriendsTimeLineDBTask.updatePosition(position, accountId,
                        groupId);
            }
        };
    ...
    private static void updatePosition(TimeLinePosition position,
        ...
        if (groupId.equals("0")) {
            ...
            HomeOtherGroupTimeLineDBTask.updatePosition(position, GlobalContext
    ...
    private static void updateCount(String msgId, int commentCount,
        ...
        String sql = "select * from " + HomeTable.HomeDataTable.TABLE_NAME
                ...
                + " order by " + HomeTable.HomeDataTable.ID + " asc limit 50";
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        while (c.moveToNext()) {
    ...
    public static MessageTimeLineData getRecentGroupData(String accountId) {
        ...
        if (groupId.equals("0")) {
            ...
            msgList = getHomeLineMsgList(accountId, position.position
                    + AppConfig.DB_CACHE_COUNT_OFFSET);
        }
        else {
            position = HomeOtherGroupTimeLineDBTask.getPosition(accountId,
                    groupId);
    ...
    public static List<MessageTimeLineData> getOtherGroupData(String accountId,
        ...
        if (!"0".equals(exceptGroupId)) {
            ...
            MessageListBean msgList = getHomeLineMsgList(accountId,
                    position.position + AppConfig.DB_CACHE_COUNT_OFFSET);
            MessageTimeLineData home = new MessageTimeLineData("0", msgList,
                    position);
            data.add(home);
        }
        
        MessageTimeLineData biGroup = HomeOtherGroupTimeLineDBTask
                .getTimeLineData(accountId, "1");
        data.add(biGroup);
