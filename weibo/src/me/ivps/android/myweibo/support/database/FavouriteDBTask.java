...
public class FavouriteDBTask {
    ...
    private static void add(List<FavBean> msgList, int page, String accountId) {
        ...
        DatabaseUtils.InsertHelper ih = new DatabaseUtils.InsertHelper(
                getWsd(), FavouriteTable.FavouriteDataTable.TABLE_NAME);
        final int mblogidColumn = ih
                .getColumnIndex(FavouriteTable.FavouriteDataTable.MBLOGID);
        final int accountidColumn = ih
                .getColumnIndex(FavouriteTable.FavouriteDataTable.ACCOUNTID);
        final int jsondataColumn = ih
                .getColumnIndex(FavouriteTable.FavouriteDataTable.JSONDATA);
        ...
        String sql = "select * from " + FavouriteTable.TABLE_NAME + " where "
                + FavouriteTable.ACCOUNTID + "  = " + accountId;
        ...
        if (c.moveToNext()) {
            try {
                ...
                getWsd().update(FavouriteTable.TABLE_NAME, cv,
                        FavouriteTable.ACCOUNTID + "=?", args);
            }
    ...
    public static FavouriteTimeLineData getFavouriteMsgList(String accountId) {
        ...
        String sql = "select * from "
                + FavouriteTable.FavouriteDataTable.TABLE_NAME + " where "
                + FavouriteTable.FavouriteDataTable.ACCOUNTID + "  = "
                + accountId + " order by "
                + FavouriteTable.FavouriteDataTable.MBLOGID + " desc";
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        while (c.moveToNext()) {
            String json = c
        ...
        sql = "select * from " + FavouriteTable.TABLE_NAME + " where "
                + FavouriteTable.ACCOUNTID + "  = " + accountId;
    ...
    static void deleteAllFavourites(String accountId) {
        ...
        sql = "delete from " + FavouriteTable.TABLE_NAME + " where "
                + FavouriteTable.ACCOUNTID + " in " + "(" + accountId + ")";
        getWsd().execSQL(sql);
    }
    ...
    private static TimeLinePosition getPosition(String accountId) {
        ...
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(FavouriteTable.TIMELINEDATA));
