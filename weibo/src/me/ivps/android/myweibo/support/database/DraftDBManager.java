...
public class DraftDBManager {
    ...
    public void insertStatus(String content, GeoBean gps, String pic,
            String accountId) {
    ...
    public void insertRepost(String content, MessageBean messageBean,
            String accountId) {
        ContentValues cv = new ContentValues();
        cv.put(DraftTable.CONTENT, content);
        cv.put(DraftTable.ACCOUNTID, accountId);
    ...
    public void insertComment(String content, MessageBean messageBean,
            String accountId) {
        ContentValues cv = new ContentValues();
        cv.put(DraftTable.CONTENT, content);
        cv.put(DraftTable.ACCOUNTID, accountId);
    ...
    public void insertReply(String content, CommentBean commentBean,
            String accountId) {
        ContentValues cv = new ContentValues();
        cv.put(DraftTable.CONTENT, content);
        cv.put(DraftTable.ACCOUNTID, accountId);
    ...
    public List<DraftListViewItemBean> getDraftList(String accountId) {
        ...
        String sql = "select * from " + DraftTable.TABLE_NAME + " where "
                + DraftTable.ACCOUNTID + "  = " + accountId + " order by "
                + DraftTable.ID + " desc";
        ...
        while (c.moveToNext()) {
            ...
            switch (type) {
                case DraftTable.TYPE_WEIBO:
                    ...
                    String gpsJson = c.getString(c
                            .getColumnIndex(DraftTable.GPS));
                ...
                case DraftTable.TYPE_REPOST:
                    ...
                    repostDraftBean.setId(c.getString(c
                            .getColumnIndex(DraftTable.ID)));
                    
                    repostDraftBean.setContent(content);
                    repostDraftBean.setAccountId(accountId);
                    
                    MessageBean messageBean = gson.fromJson(
                            c.getString(c.getColumnIndex(DraftTable.JSONDATA)),
                            MessageBean.class);
                ...
                case DraftTable.TYPE_COMMENT:
                    ...
                    commentDraftBean.setId(c.getString(c
                            .getColumnIndex(DraftTable.ID)));
                    
                    commentDraftBean.setContent(content);
                    commentDraftBean.setAccountId(accountId);
                    
                    MessageBean commentMessageBean = gson.fromJson(
                            c.getString(c.getColumnIndex(DraftTable.JSONDATA)),
                            MessageBean.class);
                ...
                case DraftTable.TYPE_REPLY:
                    ...
                    replyDraftBean.setId(c.getString(c
                            .getColumnIndex(DraftTable.ID)));
                    
                    replyDraftBean.setContent(content);
                    replyDraftBean.setAccountId(accountId);
                    
                    CommentBean commentBean = gson.fromJson(
                            c.getString(c.getColumnIndex(DraftTable.JSONDATA)),
                            CommentBean.class);
    ...
    public List<DraftListViewItemBean> removeAndGet(
            Set<String> checkedItemPosition, String acountId) {
        String[] args = checkedItemPosition.toArray(new String[0]);
        String asString = Arrays.toString(args);
        asString = asString.replace("[", "(");
        asString = asString.replace("]", ")");
        
        String sql = "delete from " + DraftTable.TABLE_NAME + " where "
                + DraftTable.ID + " in " + asString;
