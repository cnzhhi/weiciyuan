...
public class HomeOtherGroupTimeLineDBTask {
    ...
    private static void addHomeLineMsg(MessageListBean list, String accountId,
        ...
        final int mblogidColumn = ih
                .getColumnIndex(HomeOtherGroupTable.HomeOtherGroupDataTable.MBLOGID);
        final int accountidColumn = ih
                .getColumnIndex(HomeOtherGroupTable.HomeOtherGroupDataTable.ACCOUNTID);
        final int jsondataColumn = ih
                .getColumnIndex(HomeOtherGroupTable.HomeOtherGroupDataTable.JSONDATA);
        final int groupidColumn = ih
                .getColumnIndex(HomeOtherGroupTable.HomeOtherGroupDataTable.GROUPID);
    ...
    private static void reduceHomeOtherGroupTable(String accountId,
            String groupId) {
        String searchCount = "select count("
                + HomeOtherGroupTable.HomeOtherGroupDataTable.ID + ") as total"
                + " from "
                + HomeOtherGroupTable.HomeOtherGroupDataTable.TABLE_NAME
                + " where "
        ...
        if (needDeletedNumber > 0) {
            ...
            String sql = " delete from "
                    + HomeOtherGroupTable.HomeOtherGroupDataTable.TABLE_NAME
                    + " where "
                    + HomeOtherGroupTable.HomeOtherGroupDataTable.ID + " in "
                    + "( select "
                    + HomeOtherGroupTable.HomeOtherGroupDataTable.ID + " from "
                    + HomeOtherGroupTable.HomeOtherGroupDataTable.TABLE_NAME
                    + " where "
                    + HomeOtherGroupTable.HomeOtherGroupDataTable.ACCOUNTID
                    + " in " + "(" + accountId + ") " + " and "
    ...
    public static void updatePosition(TimeLinePosition position,
            String accountId, String groupId) {
        String sql = "select * from " + HomeOtherGroupTable.TABLE_NAME
                + " where " + HomeOtherGroupTable.ACCOUNTID + "  = "
                + accountId + " and " + HomeOtherGroupTable.GROUPID + " = "
                + groupId;
        ...
        if (c.moveToNext()) {
            try {
                ...
                getWsd().update(
                        ...
                        cv,
                        HomeOtherGroupTable.ACCOUNTID + "=? AND "
                                + HomeOtherGroupTable.GROUPID + " =? ", args);
            }
    ...
    static TimeLinePosition getPosition(String accountId, String groupId) {
        ...
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(HomeOtherGroupTable.TIMELINEDATA));
            if (!TextUtils.isEmpty(json)) {
                try {
                    TimeLinePosition value = gson.fromJson(json,
                            TimeLinePosition.class);
                    c.close();
                    return value;
                }
                catch (JsonSyntaxException e) {
    ...
    static MessageTimeLineData getTimeLineData(String accountId, String groupId) {
        ...
        MessageListBean msgList = get(accountId, groupId, position.position
    ...
    static void updateCount(String msgId, int commentCount, int repostCount) {
        ...
        while (c.moveToNext()) {
            String id = c
                    .getString(c
                            .getColumnIndex(HomeOtherGroupTable.HomeOtherGroupDataTable.ID));
            String json = c
                    .getString(c
                            .getColumnIndex(HomeOtherGroupTable.HomeOtherGroupDataTable.JSONDATA));
            if (!TextUtils.isEmpty(json)) {
