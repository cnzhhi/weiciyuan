...
public class CommentByMeTimeLineDBTask {
    ...
    public static void addCommentLineMsg(CommentListBean list, String accountId) {
        ...
        DatabaseUtils.InsertHelper ih = new DatabaseUtils.InsertHelper(
                getWsd(), CommentByMeTable.CommentByMeDataTable.TABLE_NAME);
        final int mblogidColumn = ih
                .getColumnIndex(CommentByMeTable.CommentByMeDataTable.MBLOGID);
    ...
    public static CommentTimeLineData getCommentLineMsgList(String accountId) {
        ...
        String sql = "select * from "
                ...
                + accountId + " order by "
                + CommentByMeTable.CommentByMeDataTable.MBLOGID
                + " desc limit " + limit;
    ...
    private static void reduceCommentTable(String accountId) {
        String searchCount = "select count("
                + CommentByMeTable.CommentByMeDataTable.ID + ") as total"
                + " from " + CommentByMeTable.CommentByMeDataTable.TABLE_NAME
    ...
    private static void updatePosition(TimeLinePosition position,
        ...
        String sql = "select * from " + CommentByMeTable.TABLE_NAME + " where "
                + CommentByMeTable.ACCOUNTID + "  = " + accountId;
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        if (c.moveToNext()) {
            try {
                ...
                getWsd().update(CommentByMeTable.TABLE_NAME, cv,
                        CommentByMeTable.ACCOUNTID + "=?", args);
            }
            catch (JsonSyntaxException e) {
            ...
            cv.put(CommentByMeTable.TIMELINEDATA, gson.toJson(position));
            getWsd().insert(CommentByMeTable.TABLE_NAME, CommentByMeTable.ID,
                    cv);
        }
    }
    ...
    private static TimeLinePosition getPosition(String accountId) {
        ...
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(CommentByMeTable.TIMELINEDATA));
            if (!TextUtils.isEmpty(json)) {
                try {
    ...
    public static void asyncReplace(final CommentListBean data,
            final String accountId) {
