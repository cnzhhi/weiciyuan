...
public class MentionCommentsTimeLineDBTask {
    ...
    public static CommentTimeLineData getCommentLineMsgList(String accountId) {
        ...
        c.close();
        CommentTimeLineData mentionTimeLineData = new CommentTimeLineData(
                result, position);
    ...
    private static void updatePosition(TimeLinePosition position,
            String accountId) {
        String sql = "select * from " + MentionCommentsTable.TABLE_NAME
                + " where " + MentionCommentsTable.ACCOUNTID + "  = "
                + accountId;
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        if (c.moveToNext()) {
    ...
    public static TimeLinePosition getPosition(String accountId) {
        ...
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(MentionCommentsTable.TIMELINEDATA));
            if (!TextUtils.isEmpty(json)) {
                try {
