...
public class DMDBTask {
    ...
    public static void asyncReplace(final DMUserListBean list,
    ...
    public static void clear(String accountId) {
        String sql = "delete from " + DMTable.TABLE_NAME + " where "
                + DMTable.ACCOUNTID + " in " + "(" + accountId + ")";
    ...
    public static DMUserListBean get(String accountId) {
        ...
        String sql = "select * from " + DMTable.TABLE_NAME + " where "
                + DMTable.ACCOUNTID + "  = " + accountId + " order by "
                + DMTable.MBLOGID + " desc limit 1";
        Cursor c = getRsd().rawQuery(sql, null);
        while (c.moveToNext()) {
            String json = c.getString(c.getColumnIndex(DMTable.JSONDATA));
            try {
                DMUserListBean value = gson
                        .fromJson(json, DMUserListBean.class);
