...
public class MentionWeiboTimeLineDBTask {
    ...
    public static MentionTimeLineData getRepostLineMsgList(String accountId) {
        ...
        String sql = "select * from " + RepostsTable.RepostDataTable.TABLE_NAME
                + " where " + RepostsTable.RepostDataTable.ACCOUNTID + "  = "
                + accountId + " order by "
                + RepostsTable.RepostDataTable.MBLOGID + " desc limit " + limit;
        Cursor c = getRsd().rawQuery(sql, null);
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(RepostsTable.RepostDataTable.JSONDATA));
        ...
        c.close();
        MentionTimeLineData mentionTimeLineData = new MentionTimeLineData(
                result, position);
    ...
    public static void addRepostLineMsg(MessageListBean list, String accountId) {
        ...
        DatabaseUtils.InsertHelper ih = new DatabaseUtils.InsertHelper(
                getWsd(), RepostsTable.RepostDataTable.TABLE_NAME);
        final int mblogidColumn = ih
                .getColumnIndex(RepostsTable.RepostDataTable.MBLOGID);
        final int accountidColumn = ih
                .getColumnIndex(RepostsTable.RepostDataTable.ACCOUNTID);
    ...
    private static void reduceRepostTable(String accountId) {
        String searchCount = "select count(" + RepostsTable.RepostDataTable.ID
                + ") as total" + " from "
    ...
    private static void replaceRepostLineMsg(MessageListBean list,
            String accountId) {
    ...
    static void deleteAllReposts(String accountId) {
        String sql = "delete from " + RepostsTable.RepostDataTable.TABLE_NAME
                + " where " + RepostsTable.RepostDataTable.ACCOUNTID + " in "
                + "(" + accountId + ")";
    ...
    private static void updatePosition(TimeLinePosition position,
            String accountId) {
        String sql = "select * from " + RepostsTable.TABLE_NAME + " where "
                + RepostsTable.ACCOUNTID + "  = " + accountId;
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        if (c.moveToNext()) {
            try {
                ...
                getWsd().update(RepostsTable.TABLE_NAME, cv,
                        RepostsTable.ACCOUNTID + "=?", args);
            }
            catch (JsonSyntaxException e) {
    ...
    public static TimeLinePosition getPosition(String accountId) {
        ...
        while (c.moveToNext()) {
            String json = c.getString(c
                    .getColumnIndex(RepostsTable.TIMELINEDATA));
            if (!TextUtils.isEmpty(json)) {
                try {
