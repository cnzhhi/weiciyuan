...
public class AtUsersDBTask {
    ...
    public static void add(SQLiteDatabase db, AtUserBean atUserBean,
            String accountId) {
        Gson gson = new Gson();
    ...
    public static List<AtUserBean> get(SQLiteDatabase db, String accountId) {
        ...
        String sql = "select * from " + AtUsersTable.TABLE_NAME + " where "
                + AtUsersTable.ACCOUNTID + "  = " + accountId + " order by "
                + AtUsersTable.ID + " desc";
    ...
    private static void reduce(SQLiteDatabase db, String accountId) {
        String searchCount = "select count(" + AtUsersTable.ID + ") as total"
                + " from " + AtUsersTable.TABLE_NAME + " where "
                + AtUsersTable.ACCOUNTID + " = " + accountId;
        int total = 0;
        ...
        if (needDeletedNumber > 0) {
            String sql = " delete from " + AtUsersTable.TABLE_NAME + " where "
                    + AtUsersTable.ID + " in " + "( select " + AtUsersTable.ID
                    + " from " + AtUsersTable.TABLE_NAME + " where "
                    + AtUsersTable.ACCOUNTID + " in " + "(" + accountId
