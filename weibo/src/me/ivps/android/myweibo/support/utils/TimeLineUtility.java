...
import me.ivps.android.myweibo.bean.AdBean;
...
import me.ivps.android.myweibo.support.lib.WeiboPatterns;
...
public class TimeLineUtility {
    ...
    public static void addLinks(TextView view) {
        ...
        if (view.getLinksClickable()) {
            view.setMovementMethod(LongClickableLinkMovementMethod
                    .getInstance());
        }
    }
    ...
    public static void filterMessage(MessageListBean value) {
        ...
        List<String> keywordFilter = FilterDBTask
                .getFilterKeywordList(FilterDBTask.TYPE_KEYWORD);
        List<String> userFilter = FilterDBTask
                .getFilterKeywordList(FilterDBTask.TYPE_USER);
        List<String> topicFilter = FilterDBTask
                .getFilterKeywordList(FilterDBTask.TYPE_TOPIC);
        List<String> sourceFilter = FilterDBTask
                .getFilterKeywordList(FilterDBTask.TYPE_SOURCE);
        ...
        while (iterator.hasNext()) {
            ...
            else if (SettingUtility.isEnableFilter()
                    && TimeLineUtility.haveFilterWord(msg, keywordFilter,
                            userFilter, topicFilter, sourceFilter)) {
                iterator.remove();
                value.removedCountPlus();
            }
            else {
    ...
    public static boolean haveFilterWord(MessageBean content,
            List<String> keywordFilter, List<String> userFilter,
            List<String> topicFilter, List<String> sourceFilter) {
        
        // if this message is sent myself, ignore it;
        if (content.getUser().getId()
        ...
        for (String filterWord : userFilter) {
            ...
            if (content.getUser() != null
                    && content.getUser().getScreen_name().equals(filterWord)) {
                return true;
            }
        ...
        for (String filterWord : topicFilter) {
            ...
            Matcher localMatcher = WeiboPatterns.TOPIC_URL.matcher(content
                    .getText());
            ...
            if (content.getRetweeted_status() != null) {
                ...
                localMatcher = WeiboPatterns.TOPIC_URL.matcher(content
        ...
        for (String filterWord : sourceFilter) {
            ...
            if (hasOriMessage
                    && filterWord.equals(content.getRetweeted_status()
                            .getSourceString())) {
                return true;
            }
        }
