...
import me.ivps.android.myweibo.dao.group.ModifyGroupMemberDao;
import me.ivps.android.myweibo.dao.relationship.FanDao;
import me.ivps.android.myweibo.dao.relationship.FriendshipsDao;
import me.ivps.android.myweibo.dao.show.ShowUserDao;
import me.ivps.android.myweibo.dao.user.RemarkDao;
import me.ivps.android.myweibo.support.database.FilterDBTask;
import me.ivps.android.myweibo.support.debug.AppLogger;
import me.ivps.android.myweibo.support.error.ErrorCode;
...
import me.ivps.android.myweibo.ui.common.CommonErrorDialogFragment;
import me.ivps.android.myweibo.ui.common.CommonProgressDialogFragment;
import me.ivps.android.myweibo.ui.interfaces.AbstractAppActivity;
import me.ivps.android.myweibo.ui.loader.AbstractAsyncNetRequestTaskLoader;
...
public class UserInfoActivity extends AbstractAppActivity {
        ...
        if (bean == null) {
            ...
            if (!TextUtils.isEmpty(id)) {
                ...
                if (!TextUtils.isEmpty(domain)) {
                    ...
                    else if (NfcAdapter.ACTION_NDEF_DISCOVERED
    ...
    private void fetchUserInfoFromServer() {
        ...
        getSupportFragmentManager().beginTransaction()
                .add(dialog, CommonProgressDialogFragment.class.getName())
                .commit();
        getSupportLoaderManager().initLoader(REFRESH_LOADER_ID, null,
                refreshCallback);
    }
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                if (getSupportFragmentManager().findFragmentByTag(
                        UserInfoFragment.class.getName()) == null) {
                    UserInfoFragment userInfoFragment = UserInfoFragment
                            .newInstance(getUser(), getToken());
                    getSupportFragmentManager()
                            .beginTransaction()
                            .replace(android.R.id.content, userInfoFragment,
                                    UserInfoFragment.class.getName()).commit();
                    getSupportFragmentManager().executePendingTransactions();
                    
                    AnimationUtility.translateFragmentY(userInfoFragment, -400,
                            0, userInfoFragment);
                }
            }
        });
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (isMyselfProfile()) {
            ...
            getMenuInflater().inflate(R.menu.actionbar_menu_myinfoactivity,
                    menu);
    ...
    public void updateRemark(String remark) {
        new UpdateRemarkTask(remark)
                .executeOnExecutor(MyAsyncTask.THREAD_POOL_EXECUTOR);
    }
    
    private UserInfoFragment getInfoFragment() {
        return ((UserInfoFragment) getSupportFragmentManager()
                .findFragmentByTag(UserInfoFragment.class.getName()));
    }
    
    private void manageGroup() {
        ManageGroupDialog dialog = new ManageGroupDialog(GlobalContext
                .getInstance().getGroup(), bean.getId());
    ...
    private class ModifyGroupMemberTask extends MyAsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ModifyGroupMemberDao dao = new ModifyGroupMemberDao(token,
                    bean.getId());
            for (String id : add) {
                try {
                    dao.add(id);
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            Toast.makeText(UserInfoActivity.this,
                    getString(R.string.modify_successfully), Toast.LENGTH_SHORT)
                    .show();
        }
    }
    ...
    private class UnFollowTask extends MyAsyncTask<Void, UserBean, UserBean> {
        ...
        protected void onPostExecute(UserBean o) {
            ...
            Toast.makeText(UserInfoActivity.this,
                    getString(R.string.unfollow_successfully),
                    Toast.LENGTH_SHORT).show();
            bean = o;
    ...
    private class FollowTask extends MyAsyncTask<Void, UserBean, UserBean> {
        ...
        protected void onCancelled(UserBean userBean) {
            super.onCancelled(userBean);
            if (e != null) {
                Toast.makeText(UserInfoActivity.this, e.getMessage(),
                        Toast.LENGTH_SHORT).show();
        ...
        protected void onPostExecute(UserBean o) {
            super.onPostExecute(o);
            Toast.makeText(UserInfoActivity.this,
                    getString(R.string.follow_successfully), Toast.LENGTH_SHORT)
                    .show();
            bean = o;
    ...
    private class RemoveFanTask extends MyAsyncTask<Void, UserBean, UserBean> {
        ...
        protected void onPostExecute(UserBean o) {
            super.onPostExecute(o);
            Toast.makeText(UserInfoActivity.this,
                    getString(R.string.remove_fan_successfully),
                    Toast.LENGTH_SHORT).show();
            bean = o;
    ...
    class UpdateRemarkTask extends MyAsyncTask<Void, UserBean, UserBean> {
        ...
        protected UserBean doInBackground(Void... params) {
            try {
                return new RemarkDao(getToken(), bean.getId(), remark)
                        .updateRemark();
            }
            catch (WeiboException e) {
                this.e = e;
        ...
        protected void onCancelled(UserBean userBean) {
            super.onCancelled(userBean);
            if (this.e != null) {
                Toast.makeText(UserInfoActivity.this, this.e.getError(),
                        Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private static class RefreshLoader extends
            AbstractAsyncNetRequestTaskLoader<UserBean> {
    ...
    private LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<UserBean>> refreshCallback = new LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<UserBean>>() {
        @Override
        public Loader<AsyncTaskLoaderResult<UserBean>> onCreateLoader(int id,
                Bundle args) {
        ...
        public void onLoadFinished(
                Loader<AsyncTaskLoaderResult<UserBean>> loader,
                AsyncTaskLoaderResult<UserBean> result) {
            UserBean data = result != null ? result.data : null;
            final WeiboException exception = result != null ? result.exception
                    : null;
            ...
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                ...
                public void run() {
                    CommonProgressDialogFragment dialog = (CommonProgressDialogFragment) getSupportFragmentManager()
                            .findFragmentByTag(
                                    CommonProgressDialogFragment.class
                                            .getName());
                    if (dialog != null) {
                    ...
                    if (exception != null) {
                        ...
                        getSupportFragmentManager()
                                ...
                                .add(userInfoActivityErrorDialog,
                                        CommonErrorDialogFragment.class
                                                .getName()).commit();
                    }
                }
            });
