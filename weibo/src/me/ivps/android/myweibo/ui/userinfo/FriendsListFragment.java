...
import me.ivps.android.myweibo.ui.actionmenu.MyFriendSingleChoiceModeListener;
import me.ivps.android.myweibo.ui.actionmenu.NormalFriendShipSingleChoiceModeListener;
...
public class FriendsListFragment extends AbstractFriendsFanListFragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        getListView().setOnItemLongClickListener(
                new FriendListOnItemLongClickListener());
    }
    ...
    private class FriendListOnItemLongClickListener implements
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view,
            ...
            if (position - 1 < getList().getUsers().size() && position - 1 >= 0) {
                if (actionMode != null) {
                    ...
                    if (getCurrentUser().getId().equals(
                            GlobalContext.getInstance().getCurrentAccountId())) {
                        actionMode = getActivity().startActionMode(
                                new MyFriendSingleChoiceModeListener(
                                        getListView(), getAdapter(),
                                        FriendsListFragment.this, bean
                                                .getUsers().get(position - 1)));
                    }
                    else {
                        actionMode = getActivity().startActionMode(
                                new NormalFriendShipSingleChoiceModeListener(
                                        getListView(), getAdapter(),
                                        FriendsListFragment.this, bean
                                                .getUsers().get(position - 1)));
                    }
