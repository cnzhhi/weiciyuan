...
import me.ivps.android.myweibo.bean.android.FavouriteTimeLineData;
...
import me.ivps.android.myweibo.support.database.FavouriteDBTask;
...
import me.ivps.android.myweibo.ui.loader.MyFavMsgLoader;
 ...
 */
public class MyFavListFragment extends
        AbstractMessageTimeLineFragment<FavListBean> implements
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if ((((MainTimeLineActivity) getActivity()).getMenuFragment())
                .getCurrentIndex() == LeftMenuFragment.FAV_INDEX) {
            ...
            for (int i = 0; i < getList().getSize(); i++) {
                if (msg.equals(getList().getItem(i))) {
                    ...
                    if (ori.getComments_count() != msg.getComments_count()
                        ...
                        FavouriteDBTask.asyncReplace(getList(), page,
                                account.getUid());
    ...
    @Override
    protected void newMsgLoaderSuccessCallback(FavListBean newValue,
            Bundle loaderArgs) {
    ...
    @Override
    protected Loader<AsyncTaskLoaderResult<FavListBean>> onCreateNewMsgLoader(
            int id, Bundle args) {
    ...
    @Override
    protected Loader<AsyncTaskLoaderResult<FavListBean>> onCreateOldMsgLoader(
            int id, Bundle args) {
        String token = GlobalContext.getInstance().getSpecialToken();
        return new MyFavMsgLoader(getActivity(), token,
    ...
    private class DBCacheTask extends
            MyAsyncTask<Void, FavouriteTimeLineData, FavouriteTimeLineData> {
