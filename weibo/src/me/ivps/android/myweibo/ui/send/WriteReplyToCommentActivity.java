...
import me.ivps.android.myweibo.othercomponent.sendweiboservice.SendReplyToCommentService;
 ...
 */
public class WriteReplyToCommentActivity extends
        AbstractWriteActivity<CommentBean> {
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            replyDraftBean = (ReplyDraftBean) savedInstanceState
                    .getParcelable("replyDraftBean");
        }
    }
            ...
            if (!TextUtils.isEmpty(action)) {
                if (action.equals(WriteReplyToCommentActivity.ACTION_DRAFT)) {
                    handleDraftOperation(intent);
                        ...
                        .equals(WriteReplyToCommentActivity.ACTION_SEND_FAILED)) {
                    handleFailedOperation(intent);
                }
                else if (action
                        .equals(WriteReplyToCommentActivity.ACTION_NOTIFICATION_REPLY)) {
    ...
    public static Intent newIntentFromNotification(Context context,
            AccountBean account, CommentBean oriMsg) {
        Intent intent = new Intent(context, WriteReplyToCommentActivity.class);
    ...
    protected boolean canShowSaveDraftDialog() {
        ...
        else if (!replyDraftBean.getContent().equals(
    ...
    public void saveToDraft() {
        if (!TextUtils.isEmpty(getEditTextView().getText().toString())) {
            DraftDBManager.getInstance().insertReply(
                    getEditTextView().getText().toString(), bean,
    ...
    protected void send() {
        if (canSend()) {
            String content = ((EditText) findViewById(R.id.status_new_content))
                    .getText().toString();
            Intent intent = SendReplyToCommentService.newIntent(
                    getCurrentAccountBean(), bean, content,
    ...
    private String repost() {
        String content = ((EditText) findViewById(R.id.status_new_content))
                .getText().toString();
        String msgContent = "//@" + bean.getUser().getScreen_name() + ": "
                + bean.getText();
    ...
    protected AccountBean getCurrentAccountBean() {
        if (WriteReplyToCommentActivity.ACTION_NOTIFICATION_REPLY
                .equals(getIntent().getAction())
                || WriteReplyToCommentActivity.ACTION_SEND_FAILED
                        .equals(getIntent().getAction())) {
