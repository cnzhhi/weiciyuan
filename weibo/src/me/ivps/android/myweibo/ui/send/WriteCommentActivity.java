...
import me.ivps.android.myweibo.dao.send.RepostNewMsgDao;
import me.ivps.android.myweibo.othercomponent.sendweiboservice.SendCommentService;
import me.ivps.android.myweibo.othercomponent.sendweiboservice.SendRepostService;
...
public class WriteCommentActivity extends AbstractWriteActivity<ItemBean> {
            ...
            if (!TextUtils.isEmpty(action)) {
                if (action.equals(WriteCommentActivity.ACTION_DRAFT)) {
                    ...
                    handleFailedOperation(intent);
                        ...
                        .equals(WriteCommentActivity.ACTION_NOTIFICATION_COMMENT)) {
    ...
    public static Intent startBecauseSendFailed(Context context,
            ...
            CommentDraftBean draft, boolean comment_ori, String failedReason) {
    ...
    public static Intent newIntentFromNotification(Context context,
            AccountBean account, MessageBean msg) {
        Intent intent = new Intent(context, WriteCommentActivity.class);
    ...
    private void handleFailedOperation(Intent intent) {
        token = ((AccountBean) intent.getParcelableExtra("account"))
                .getAccess_token();
        ...
        commentDraftBean = (CommentDraftBean) intent
    ...
    private void handleDraftOperation(Intent intent) {
        ...
        commentDraftBean = (CommentDraftBean) getIntent().getParcelableExtra(
                "draft");
    ...
    protected boolean canShowSaveDraftDialog() {
        ...
        else if (!commentDraftBean.getContent().equals(
    ...
    public void saveToDraft() {
        if (!TextUtils.isEmpty(getEditTextView().getText().toString())) {
            DraftDBManager.getInstance().insertComment(
                    getEditTextView().getText().toString(), msg,
    ...
    protected boolean canSend() {
        boolean haveContent = !TextUtils.isEmpty(getEditTextView().getText()
                .toString());
        ...
        if (haveContent && haveToken && contentNumBelow140) {
            ...
            if (!haveContent && !haveToken) {
                ...
                getEditTextView().setError(
                        getString(R.string.content_cant_be_empty));
            }
            else if (!haveToken) {
            ...
            if (!contentNumBelow140) {
                getEditTextView().setError(
                        getString(R.string.content_words_number_too_many));
            }
        }
    ...
    protected void send() {
        if (!enableRepost.isChecked()) {
            String content = ((EditText) findViewById(R.id.status_new_content))
                    .getText().toString();
            Intent intent = SendCommentService.newIntent(
                    getCurrentAccountBean(), msg, content,
                    enableCommentOri.isChecked());
            startService(intent);
            finish();
        }
        else {
    ...
    private void repost() {
        ...
        if (msg.getRetweeted_status() != null) {
            String msgContent = "//@" + msg.getUser().getScreen_name() + ": "
                    + msg.getText();
        ...
        intent.putExtra("accountId", GlobalContext.getInstance()
                .getCurrentAccountId());
        startService(intent);
        finish();
    }
    ...
    protected AccountBean getCurrentAccountBean() {
        if (WriteCommentActivity.ACTION_NOTIFICATION_COMMENT.equals(getIntent()
                .getAction())
                || WriteCommentActivity.ACTION_SEND_FAILED.equals(getIntent()
                        .getAction())) {
            AccountBean accountBean = ((AccountBean) getIntent()
