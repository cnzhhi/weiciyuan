...
import me.ivps.android.myweibo.bean.MessageReCmtCountBean;
import me.ivps.android.myweibo.bean.UserBean;
import me.ivps.android.myweibo.bean.android.AsyncTaskLoaderResult;
import me.ivps.android.myweibo.bean.android.MessageTimeLineData;
import me.ivps.android.myweibo.bean.android.TimeLinePosition;
import me.ivps.android.myweibo.dao.maintimeline.TimeLineReCmtCountDao;
import me.ivps.android.myweibo.othercomponent.WifiAutoDownloadPictureRunnable;
import me.ivps.android.myweibo.support.database.FriendsTimeLineDBTask;
import me.ivps.android.myweibo.support.debug.AppLogger;
import me.ivps.android.myweibo.support.error.WeiboException;
import me.ivps.android.myweibo.support.lib.HeaderListView;
import me.ivps.android.myweibo.support.lib.LogOnExceptionScheduledExecutor;
...
import me.ivps.android.myweibo.support.utils.BundleArgsConstants;
import me.ivps.android.myweibo.support.utils.GlobalContext;
import me.ivps.android.myweibo.support.utils.Utility;
import me.ivps.android.myweibo.ui.adapter.AbstractAppListAdapter;
...
import me.ivps.android.myweibo.ui.loader.FriendsMsgLoader;
import me.ivps.android.myweibo.ui.main.LeftMenuFragment;
import me.ivps.android.myweibo.ui.main.MainTimeLineActivity;
import me.ivps.android.myweibo.ui.send.WriteWeiboActivity;
...
/**
 * User: qii Date: 12-7-29 Weibo home/friend timeline
 */
public class FriendsTimeLineFragment extends
    ...
    private static final String ARGUMENTS_ACCOUNT_EXTRA = FriendsTimeLineFragment.class
            .getName() + ":account_extra";
    private static final String ARGUMENTS_USER_EXTRA = FriendsTimeLineFragment.class
            .getName() + ":userBean_extra";
    private static final String ARGUMENTS_TOKEN_EXTRA = FriendsTimeLineFragment.class
            .getName() + ":token_extra";
    ...
    private void startDownloadingOtherPicturesOnWifiNetworkEnvironment() {
        if (backgroundWifiDownloadPicThread == null
            ...
            int listVewOrientation = ((VelocityListView) getListView())
    ...
    private void setListViewPositionFromPositionsCache() {
        final TimeLinePosition timeLinePosition = positionCache
                .get(currentGroupId);
        AppLogger
    ...
    public void buildActionBarNav() {
        ...
        getActivity().getActionBar().setListNavigationCallbacks(navAdapter,
                new ActionBar.OnNavigationListener() {
                    @Override
                    public boolean onNavigationItemSelected(int which,
                            long itemId) {
    ...
    private void addNewDataAndRememberPositionAutoRefresh(
            final MessageListBean newValue) {
        ...
        if (getActivity() != null && newValue.getSize() > 0) {
            ...
            if (hasActionMode()) {
                AppLogger
                        .v("ListView have ActionMode, skip notifyDataSetChanged");
                return;
            }
            
            final HeaderListView headerListView = (HeaderListView) getListView();
            View firstChildView = getListView().getChildAt(0);
            boolean isFirstViewHeader = headerListView
                    .isThisViewHeader(firstChildView);
            ...
            if (isFirstViewHeader && !headerListView.isInTouchByUser()) {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    int currentAdapterItemPosition = firstAdapterItemPosition
                            + i - listView.getHeaderViewsCount();
                    
                    long currentAdapterItemId = getAdapter().getItemId(
                            currentAdapterItemPosition);
                    int childViewTop = childView.getTop();
                    Bitmap bitmap = Utility.getBitmapFromView(childView);
                    
                    Pair<Long, Bitmap> pair = new Pair<Long, Bitmap>(
                            currentAdapterItemId, bitmap);
                ...
                getListView().getViewTreeObserver().addOnPreDrawListener(
                        new ViewTreeObserver.OnPreDrawListener() {
                            ...
                            public boolean onPreDraw() {
                                ...
                                Set<Long> previousViewsId = previousViews
                                        .keySet();
                                ...
                                for (int i = 0; i < childCount; i++) {
                                    ...
                                    if (previousViewsId
                                            .contains(currentAdapterItemId)) {
                                        somePreviousViewsAreStillInScreen = true;
                                        deltaY = childView.getTop()
                                                - previousViewsTop
            ...
            if (isFirstViewHeader && headerListView.isInTouchByUser()) {
                ...
                Utility.setListViewItemPosition(getListView(),
                        positionAfterRefresh, 1, endAction);
                return;
            }
            ...
            View firstAdapterItemView = Utility
                    .getListViewFirstAdapterItemView(getListView());
            final int top = (firstAdapterItemView == null) ? 0
                    : firstAdapterItemView.getTop();
            
            getAdapter().notifyDataSetChanged();
            int finalSize = getList().getSize();
            final int positionAfterRefresh = index + finalSize - initSize;
            Utility.setListViewItemPosition(getListView(),
                    positionAfterRefresh, top, endAction);
        }
    }
    ...
    private void addNewDataAndRememberPosition(final MessageListBean newValue) {
        ...
        if (getActivity() != null && newValue.getSize() > 0) {
            MessageBean previousFirstItem = getList().getSize() > 0 ? getList()
                    .getItem(0) : null;
            int index = getListView().getFirstVisiblePosition();
            getList().addNewData(newValue);
            getAdapter().notifyDataSetChanged();
            int finalSize = getList().getSize();
            int positionAfterRefresh = index + finalSize - initSize
                    + getListView().getHeaderViewsCount();
            // make sure isMiddleUnreadItem item is visible at first ListView
            // visible position
            if (previousFirstItem != null) {
                int itemPositionAfterRefresh = getList().getItemList().indexOf(
                        previousFirstItem);
                if (getList().getItem(itemPositionAfterRefresh - 1)
                        .isMiddleUnreadItem()) {
            ...
            // use 1 px to show newMsgTipBar
            Utility.setListViewItemPosition(getListView(),
                    positionAfterRefresh, 1, new Runnable() {
    ...
    private void updateTimeLineMessageCommentAndRepostData(
        ...
        for (int i = 0; i < getList().getSize(); i++) {
            ...
            MessageReCmtCountBean count = messageReCmtCountBeanHashMap.get(msg
                    .getId());
            if (count != null) {
    ...
    protected Loader<AsyncTaskLoaderResult<MessageListBean>> onCreateMiddleMsgLoader(
        ...
        String token = accountBean.getAccess_token();
        return new FriendsMsgLoader(getActivity(), accountId, token,
                currentGroupId, middleBeginId, middleEndId);
    }
