...
import me.ivps.android.myweibo.othercomponent.AppNotificationCenter;
...
import me.ivps.android.myweibo.ui.actionmenu.CommentFloatingMenu;
import me.ivps.android.myweibo.ui.actionmenu.CommentSingleChoiceModeListener;
import me.ivps.android.myweibo.ui.adapter.CommentListAdapter;
import me.ivps.android.myweibo.ui.basefragment.AbstractTimeLineFragment;
import me.ivps.android.myweibo.ui.interfaces.IRemoveItem;
import me.ivps.android.myweibo.ui.loader.CommentsToMeDBLoader;
import me.ivps.android.myweibo.ui.loader.CommentsToMeMsgLoader;
import me.ivps.android.myweibo.ui.main.CommentsTimeLine;
 ...
 */
public class CommentsToMeTimeLineFragment extends
        AbstractTimeLineFragment<CommentListBean> implements IRemoveItem {
    
    private static final String ARGUMENTS_ACCOUNT_EXTRA = CommentsToMeTimeLineFragment.class
            .getName() + ":account_extra";
    private static final String ARGUMENTS_USER_EXTRA = CommentsToMeTimeLineFragment.class
            .getName() + ":userBean_extra";
    private static final String ARGUMENTS_TOKEN_EXTRA = CommentsToMeTimeLineFragment.class
            .getName() + ":token_extra";
    private static final String ARGUMENTS_DATA_EXTRA = CommentsToMeTimeLineFragment.class
            .getName() + ":msg_extra";
    private static final String ARGUMENTS_TIMELINE_POSITION_EXTRA = CommentsToMeTimeLineFragment.class
            .getName() + ":timeline_position_extra";
    ...
    public static CommentsToMeTimeLineFragment newInstance(
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (getActivity().isChangingConfigurations()) {
            ...
            outState.putSerializable(ARGUMENTS_TIMELINE_POSITION_EXTRA,
                    timeLinePosition);
        }
    }
    ...
    private void saveTimeLinePositionToDB() {
        ...
        if (!current.isEmpty()) {
            ...
            AppLogger.i("Current ListView position first visible item id "
                    + current.firstItemId + " , save to memory cache");
        }
        else {
            AppLogger
        ...
        if (timeLinePosition != null) {
            CommentToMeTimeLineDBTask.asyncUpdatePosition(timeLinePosition,
                    accountBean.getUid());
        }
    }
    ...
    private void setActionBarTabCount(int count) {
        ...
        if (CommentsToMeTimeLineFragment.class.getName().equals(tabTag)) {
            ...
            TextView countTV = (TextView) customView
                    .findViewById(R.id.tv_home_count);
    ...
    private void setListViewPositionFromPositionsCache() {
        AppLogger
                .i("Memory cached position first visible item id "
                        + (timeLinePosition != null ? timeLinePosition.firstItemId
                                : 0));
        int position = timeLinePosition != null ? timeLinePosition
                .getPosition(bean) : 0;
        int top = timeLinePosition != null ? timeLinePosition.top : 0;
        AppLogger
                .i("Set ListView position from memory cached position position "
                        + position + " top " + top);
        Utility.setListViewAdapterPosition(getListView(), position, top,
                new Runnable() {
                    @Override
                    public void run() {
    ...
    protected void buildListAdapter() {
        CommentListAdapter adapter = new CommentListAdapter(this, getList()
    ...
    protected void listViewItemClick(AdapterView parent, View view,
        ...
        if (!clearActionModeIfOpen()) {
            CommentFloatingMenu menu = new CommentFloatingMenu(getList()
                    .getItem(position));
    ...
    /** unused **/
        ...
        if (getActivity() == null || newValue.getSize() == 0) {
            AppLogger
                    .i("Activity is destroyed or new data count is zero, give up");
            return;
        }
        
        final boolean isDataSourceEmpty = getList().getSize() == 0;
        TimeLinePosition previousPosition = Utility
                .getCurrentPositionFromListView(getListView());
        ...
        if (isDataSourceEmpty) {
            ...
            AppLogger.i("Previous first visible item id "
            ...
            newMsgTipBar
                    .setValue(CommentsToMeTimeLineFragment.this.timeLinePosition.newMsgIds);
            int positionInAdapter = Utility.getAdapterPositionFromItemId(
                    getAdapter(), previousPosition.firstItemId);
            // use 1 px to show newMsgTipBar
            AppLogger.i("ListView restore to previous position "
                    + positionInAdapter);
            ...
            Utility.setListViewAdapterPosition(getListView(),
                    positionInAdapter, previousPosition.top - 1, null);
        }
        
        showUIUnreadCount(CommentsToMeTimeLineFragment.this.timeLinePosition.newMsgIds
                .size());
    ...
    protected void oldMsgLoaderSuccessCallback(CommentListBean newValue) {
        if (newValue != null && newValue.getItemList().size() > 1) {
            ...
            getAdapter().notifyDataSetChanged();
            CommentToMeTimeLineDBTask.asyncReplace(getList(),
                    accountBean.getUid());
        }
    }
    ...
    private LoaderManager.LoaderCallbacks<CommentTimeLineData> dbCallback = new LoaderManager.LoaderCallbacks<CommentTimeLineData>() {
        ...
        public Loader<CommentTimeLineData> onCreateLoader(int id, Bundle args) {
            ...
            return new CommentsToMeDBLoader(getActivity(), GlobalContext
    ...
    protected Loader<AsyncTaskLoaderResult<CommentListBean>> onCreateNewMsgLoader(
        ...
        return new CommentsToMeMsgLoader(getActivity(), accountId, token,
                sinceId, null);
    }
    ...
    protected Loader<AsyncTaskLoaderResult<CommentListBean>> onCreateOldMsgLoader(
        ...
        return new CommentsToMeMsgLoader(getActivity(), accountId, token, null,
                maxId);
    }
    
    private AppNotificationCenter.Callback callback = new AppNotificationCenter.Callback() {
        @Override
        public void unreadCommentsChanged(AccountBean account,
            ...
            AppLogger.i("Receive new unread comments, comment count "
                    + data.getSize());
            
            addUnreadMessage(data);
            clearUnreadComment(AppNotificationCenter.getInstance()
                    .getUnreadBean(account));
        }
    };
    ...
    private void clearUnreadComment(final UnreadBean data) {
        new MyAsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                try {
                    new ClearUnreadDao(GlobalContext.getInstance()
                            .getAccountBean().getAccess_token())
                            .clearCommentUnread(data, GlobalContext
