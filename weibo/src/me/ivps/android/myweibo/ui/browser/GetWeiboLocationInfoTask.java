...
import me.ivps.android.myweibo.dao.map.MapDao;
...
public class GetWeiboLocationInfoTask extends MyAsyncTask<Void, String, Bitmap> {
    ...
    public GetWeiboLocationInfoTask(Activity activity, GeoBean geoBean,
            ImageView mapView, TextView location) {
    ...
    protected void onPreExecute() {
        ...
        location.setText(String.valueOf(geoBean.getLat() + ","
                + geoBean.getLon()));
    }
    ...
    protected Bitmap doInBackground(Void... params) {
        if (Utility.isGPSLocationCorrect(geoBean)) {
            String gpsLocationString = new GoogleGeoCoderDao(activity, geoBean)
                    .get();
            
            try {
                if (TextUtils.isEmpty(gpsLocationString)) {
                    publishProgress(new BaiduGeoCoderDao(geoBean.getLat(),
                            geoBean.getLon()).get());
                }
            }
        ...
        MapDao dao = new MapDao(GlobalContext.getInstance().getSpecialToken(),
                geoBean.getLat(), geoBean.getLon());
        try {
            return dao.getMap();
        }
        catch (WeiboException e) {
