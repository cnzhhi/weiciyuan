...
import me.ivps.android.myweibo.dao.destroy.DestroyCommentDao;
import me.ivps.android.myweibo.support.asyncdrawable.IWeiciyuanDrawable;
import me.ivps.android.myweibo.support.asyncdrawable.MsgDetailReadWorker;
import me.ivps.android.myweibo.support.asyncdrawable.TimeLineBitmapDownloader;
...
import me.ivps.android.myweibo.support.gallery.GalleryAnimationActivity;
import me.ivps.android.myweibo.support.lib.AnimationRect;
import me.ivps.android.myweibo.support.lib.ClickableTextViewMentionLinkOnTouchListener;
import me.ivps.android.myweibo.support.lib.MyAsyncTask;
import me.ivps.android.myweibo.support.lib.ProfileTopAvatarImageView;
import me.ivps.android.myweibo.support.lib.SwipeFrameLayout;
import me.ivps.android.myweibo.support.lib.WeiboDetailImageView;
import me.ivps.android.myweibo.support.lib.pulltorefresh.PullToRefreshBase;
import me.ivps.android.myweibo.support.lib.pulltorefresh.PullToRefreshListView;
import me.ivps.android.myweibo.support.settinghelper.SettingUtility;
import me.ivps.android.myweibo.support.utils.AppEventAction;
...
import me.ivps.android.myweibo.ui.actionmenu.CommentSingleChoiceModeListener;
import me.ivps.android.myweibo.ui.actionmenu.StatusSingleChoiceModeListener;
import me.ivps.android.myweibo.ui.adapter.BrowserWeiboMsgCommentAndRepostAdapter;
import me.ivps.android.myweibo.ui.interfaces.AbstractAppFragment;
import me.ivps.android.myweibo.ui.interfaces.IRemoveItem;
import me.ivps.android.myweibo.ui.loader.CommentsByIdMsgLoader;
import me.ivps.android.myweibo.ui.loader.RepostByIdMsgLoader;
import me.ivps.android.myweibo.ui.userinfo.UserInfoActivity;
 ...
 */
public class BrowserWeiboMsgFragment extends AbstractAppFragment implements
        IRemoveItem {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        switch (getCurrentState(savedInstanceState)) {
            case FIRST_TIME_START:
                if (Utility.isTaskStopped(updateMsgTask)) {
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            updateMsgTask = new UpdateMessageTask(
                                    BrowserWeiboMsgFragment.this,
                                    layout.content, layout.recontent, msg,
                                    false);
                            updateMsgTask
            ...
            case ACTIVITY_DESTROY_AND_CREATE:
                ...
                commentList.replaceAll((CommentListBean) savedInstanceState
                        .getParcelable("commentList"));
                repostList.replaceAll((RepostListBean) savedInstanceState
                        .getParcelable("repostList"));
                buildViewData(true);
        ...
        if (loader != null) {
            getLoaderManager().initLoader(NEW_COMMENT_LOADER_ID, null,
                    commentMsgCallback);
        }
        
        loader = getLoaderManager().getLoader(OLD_COMMENT_LOADER_ID);
        if (loader != null) {
            getLoaderManager().initLoader(OLD_COMMENT_LOADER_ID, null,
                    commentMsgCallback);
        }
    }
    ...
    public void onResume() {
        ...
        LocalBroadcastManager.getInstance(getActivity()).registerReceiver(
                ...
                new IntentFilter(AppEventAction
                        .buildSendCommentOrReplySuccessfullyAction(msg)));
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        PullToRefreshListView pullToRefreshListView = new PullToRefreshListView(
                getActivity());
        ...
        View header = inflater.inflate(R.layout.browserweibomsgfragment_layout,
                listView, false);
        listView.addHeaderView(header);
        
        View switchView = inflater.inflate(
                R.layout.browserweibomsgfragment_switch_list_type_header,
                listView, false);
        ...
        View progressHeaderLayout = inflater.inflate(
                R.layout.browserweibomsgfragment_progress_header, listView,
                false);
        ...
        View emptyLayout = inflater.inflate(
                R.layout.browserweibomsgfragment_empty_header, listView, false);
    ...
    private void initView(View view, Bundle savedInstanceState) {
        ...
        layout.avatar = (ProfileTopAvatarImageView) view
                .findViewById(R.id.avatar);
        layout.content_pic = (WeiboDetailImageView) view
                .findViewById(R.id.content_pic);
        layout.content_pic_multi = (GridLayout) view
                .findViewById(R.id.content_pic_multi);
        layout.repost_pic = (WeiboDetailImageView) view
                .findViewById(R.id.repost_content_pic);
        layout.repost_pic_multi = (GridLayout) view
                .findViewById(R.id.repost_content_pic_multi);
        
        layout.repost_layout = (LinearLayout) view
                .findViewById(R.id.repost_layout);
    }
    ...
    public void buildViewData(final boolean refreshPic) {
        ...
        if (msg.getUser() != null) {
            if (TextUtils.isEmpty(msg.getUser().getRemark())) {
                ...
                layout.username.setText(msg.getUser().getScreen_name() + "("
                        + msg.getUser().getRemark() + ")");
            }
            
            TimeLineBitmapDownloader.getInstance().downloadAvatar(
                    layout.avatar.getImageView(), msg.getUser());
        }
        ...
        if (msg.getGeo() != null) {
            ...
            if (Utility.isTaskStopped(geoTask)) {
                geoTask = new GetWeiboLocationInfoTask(getActivity(),
                        msg.getGeo(), layout.mapView, layout.location);
        ...
        if (msg.havePicture() && msg.getRetweeted_status() == null) {
            displayPictures(msg, layout.content_pic_multi, layout.content_pic,
                    refreshPic);
        }
        
        final MessageBean repostMsg = msg.getRetweeted_status();
        
        layout.repost_layout.setVisibility(repostMsg != null ? View.VISIBLE
                : View.GONE);
        ...
        if (repostMsg != null) {
            ...
            if (repostMsg.getUser() != null) {
                layout.recontent
                        .setText(repostMsg.getListViewSpannableString());
                buildRepostCount();
            }
            else {
                layout.recontent
            ...
            if (repostMsg.havePicture()) {
                displayPictures(repostMsg, layout.repost_pic_multi,
                        layout.repost_pic, refreshPic);
            }
        }
        ...
        ((BrowserWeiboMsgActivity) getActivity()).updateCommentCount(msg
                .getComments_count());
        ((BrowserWeiboMsgActivity) getActivity()).updateRepostCount(msg
                .getReposts_count());
    }
    ...
    private void buildRepostCount() {
        ...
        if (repostBean.getComments_count() > 0) {
            ...
            layout.comment_count.setText(String.valueOf(repostBean
                    .getComments_count()));
        }
        else {
        ...
        if (repostBean.getReposts_count() > 0) {
            ...
            layout.repost_count.setText(String.valueOf(repostBean
                    .getReposts_count()));
        }
        else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_refresh:
                if (Utility.isTaskStopped(updateMsgTask)) {
                    updateMsgTask = new UpdateMessageTask(
                            BrowserWeiboMsgFragment.this, layout.content,
                            layout.recontent, msg, true);
                    updateMsgTask
                            .executeOnExecutor(MyAsyncTask.THREAD_POOL_EXECUTOR);
                }
                if (isCommentList) {
    ...
    public void removeItem(int position) {
        ...
        if (removeTask == null
            ...
            removeTask = new RemoveTask(GlobalContext.getInstance()
                    .getSpecialToken(), commentList.getItemList().get(position)
                    .getId(), position);
    ...
    private View.OnClickListener repostContentOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (isNotLink && !isDeleted) {
                startActivity(BrowserWeiboMsgActivity.newIntent(msg
                        .getRetweeted_status(), GlobalContext.getInstance()
                        .getSpecialToken()));
            }
            else if (isNotLink && isDeleted) {
                Toast.makeText(getActivity(),
                        getString(R.string.cant_open_deleted_weibo),
    ...
    private View.OnClickListener locationInfoOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            String geoUriString = "geo:" + bean.getLat() + "," + bean.getLon()
                    + "?q=" + layout.location.getText();
    ...
    private AdapterView.OnItemLongClickListener commentOnItemLongClickListener = new AdapterView.OnItemLongClickListener() {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view,
                int position, long id) {
            if (position - listView.getHeaderViewsCount() < commentList
                    .getSize()
                ...
                if (actionMode != null) {
                    ...
                    actionMode = getActivity()
                            .startActionMode(
                                    new CommentSingleChoiceModeListener(
                                            getListView(),
                                            adapter,
                                            BrowserWeiboMsgFragment.this,
                                            commentList
    ...
    private AdapterView.OnItemClickListener repostOnItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position,
            ...
            if (position - listView.getHeaderViewsCount() < repostList
                    .getSize() && position >= listView.getHeaderViewsCount()) {
                startActivity(BrowserWeiboMsgActivity.newIntent(
                        repostList.getItemList().get(
        ...
        public void onScroll(AbsListView view, int firstVisibleItem,
            ...
            if (getListView().getLastVisiblePosition() > 7
                ...
                if (isCommentList) {
                    ...
                    if (getListView().getLastVisiblePosition() > commentList
                            .getSize() - 3) {
                        loadOldCommentData();
                    }
                }
                else {
                    if (getListView().getLastVisiblePosition() > repostList
                            .getSize() - 3) {
                        loadOldRepostData();
                    }
                }
    ...
    public void loadNewCommentData() {
        ...
        getLoaderManager().restartLoader(NEW_COMMENT_LOADER_ID, null,
                commentMsgCallback);
    }
    ...
    public void loadNewRepostData() {
        ...
        getLoaderManager().restartLoader(NEW_REPOST_LOADER_ID, null,
                repostMsgCallback);
    }
    
    public void loadOldCommentData() {
        if (getLoaderManager().getLoader(OLD_COMMENT_LOADER_ID) != null
                || !canLoadOldCommentData) {
            return;
        }
        showFooterView();
        getLoaderManager().destroyLoader(NEW_COMMENT_LOADER_ID);
        getLoaderManager().restartLoader(OLD_COMMENT_LOADER_ID, null,
                commentMsgCallback);
    }
    ...
    public void loadOldRepostData() {
        ...
        getLoaderManager().restartLoader(OLD_REPOST_LOADER_ID, null,
                repostMsgCallback);
    }
    ...
    protected LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<CommentListBean>> commentMsgCallback = new LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<CommentListBean>>() {
        ...
        @Override
        public Loader<AsyncTaskLoaderResult<CommentListBean>> onCreateLoader(
                int id, Bundle args) {
            ...
            switch (id) {
                case NEW_COMMENT_LOADER_ID:
                    String sinceId = null;
                    return new CommentsByIdMsgLoader(getActivity(),
                            msg.getId(), token, sinceId, null);
                case OLD_COMMENT_LOADER_ID:
                    String maxId = null;
                    if (commentList.getItemList().size() > 0) {
                        maxId = commentList.getItemList()
                                .get(commentList.getItemList().size() - 1)
                                .getId();
                    }
                    return new CommentsByIdMsgLoader(getActivity(),
                            msg.getId(), token, null, maxId);
            }
        ...
        public void onLoadFinished(
                Loader<AsyncTaskLoaderResult<CommentListBean>> loader,
            ...
            switch (loader.getId()) {
                case NEW_COMMENT_LOADER_ID:
                    ...
                    if (Utility.isAllNotNull(exception)) {
                        Toast.makeText(getActivity(), exception.getError(),
                                Toast.LENGTH_SHORT).show();
                    }
                    else {
    ...
    protected LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<RepostListBean>> repostMsgCallback = new LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<RepostListBean>>() {
        ...
        @Override
        public Loader<AsyncTaskLoaderResult<RepostListBean>> onCreateLoader(
                int id, Bundle args) {
            String token = GlobalContext.getInstance().getSpecialToken();
            switch (id) {
                case NEW_REPOST_LOADER_ID:
                    String sinceId = null;
                    return new RepostByIdMsgLoader(getActivity(), msg.getId(),
                            token, sinceId, null);
                case OLD_REPOST_LOADER_ID:
                    String maxId = null;
                    
                    if (repostList.getSize() > 0) {
                        maxId = repostList.getItemList()
                                .get(repostList.getSize() - 1).getId();
                    }
        ...
        @Override
        public void onLoadFinished(
                Loader<AsyncTaskLoaderResult<RepostListBean>> loader,
            ...
            if (data != null) {
                Utility.buildTabCount(repostTab, getString(R.string.repost),
                        data.getTotal_number());
                ((BrowserWeiboMsgActivity) getActivity())
                        .updateRepostCount(data.getTotal_number());
            }
