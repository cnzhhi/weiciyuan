...
import me.ivps.android.myweibo.dao.topic.TopicDao;
...
import me.ivps.android.myweibo.ui.loader.SearchTopicByNameLoader;
 ...
 */
public class SearchTopicByNameFragment extends
        AbstractMessageTimeLineFragment<TopicResultListBean> {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        switch (getCurrentState(savedInstanceState)) {
            ...
            case ACTIVITY_DESTROY_AND_CREATE:
                ...
                getList().addNewData(
                        (TopicResultListBean) savedInstanceState
                                .getParcelable("bean"));
    ...
    @Override
    protected void newMsgLoaderSuccessCallback(TopicResultListBean newValue,
            Bundle loaderArgs) {
    ...
    private class FollowTopicTask extends MyAsyncTask<Void, Boolean, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            try {
                return new TopicDao(GlobalContext.getInstance()
                        .getSpecialToken()).follow(q);
            }
        ...
        protected void onPostExecute(Boolean aBoolean) {
            ...
            if (aBoolean) {
                Toast.makeText(getActivity(),
                        getString(R.string.follow_topic_successfully),
                        Toast.LENGTH_SHORT).show();
            }
            else {
                Toast.makeText(getActivity(),
                        getString(R.string.follow_topic_failed),
                        Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private class UnFollowTopicTask extends MyAsyncTask<Void, Boolean, Boolean> {
        ...
        protected void onPostExecute(Boolean aBoolean) {
            ...
            if (aBoolean) {
                Toast.makeText(getActivity(),
                        getString(R.string.unfollow_topic_successfully),
                        Toast.LENGTH_SHORT).show();
            }
            else {
                Toast.makeText(getActivity(),
                        getString(R.string.unfollow_topic_failed),
                        Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    @Override
    protected Loader<AsyncTaskLoaderResult<TopicResultListBean>> onCreateNewMsgLoader(
            int id, Bundle args) {
        String token = GlobalContext.getInstance().getSpecialToken();
        String word = this.q;
        page = 1;
        return new SearchTopicByNameLoader(getActivity(), token, word,
                String.valueOf(page));
    }
    
    @Override
    protected Loader<AsyncTaskLoaderResult<TopicResultListBean>> onCreateOldMsgLoader(
            int id, Bundle args) {
        String token = GlobalContext.getInstance().getSpecialToken();
        String word = this.q;
        return new SearchTopicByNameLoader(getActivity(), token, word,
