...
import me.ivps.android.myweibo.othercomponent.AppNewMsgAlarm;
...
public class NotificationFragment extends PreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        preferenceList
                .add(findPreference(SettingActivity.DISABLE_FETCH_AT_NIGHT));
        preferenceList.add(findPreference(SettingActivity.FREQUENCY));
        preferenceList
                .add(findPreference(SettingActivity.ENABLE_COMMENT_TO_ME));
        preferenceList
                .add(findPreference(SettingActivity.ENABLE_MENTION_COMMENT_TO_ME));
        preferenceList
                .add(findPreference(SettingActivity.ENABLE_MENTION_TO_ME));
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == Activity.RESULT_OK) {
            ...
            sharedPref.edit()
                    .putString(SettingActivity.ENABLE_RINGTONE, ringTonePath)
                    .commit();
            buildSummary();
        }
    }
    ...
    private void buildSummary() {
        if (SettingUtility.getEnableFetchMSG()) {
            String value = PreferenceManager.getDefaultSharedPreferences(
                    getActivity()).getString(SettingActivity.FREQUENCY, "1");
            frequency.setSummary(getActivity().getResources().getStringArray(
                    R.array.frequency)[Integer.valueOf(value) - 1]);
        }
        else {
