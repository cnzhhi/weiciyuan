...
import me.ivps.android.myweibo.support.database.DMDBTask;
...
import me.ivps.android.myweibo.ui.adapter.DMUserListAdapter;
import me.ivps.android.myweibo.ui.basefragment.AbstractTimeLineFragment;
import me.ivps.android.myweibo.ui.loader.DMUserLoader;
...
public class DMUserListFragment extends
        AbstractTimeLineFragment<DMUserListBean> implements
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        switch (getCurrentState(savedInstanceState)) {
            ...
            case ACTIVITY_DESTROY_AND_CREATE:
                bean.addNewData((DMUserListBean) savedInstanceState
                        .getParcelable("bean"));
        ...
        if ((((MainTimeLineActivity) getActivity()).getMenuFragment())
                .getCurrentIndex() == LeftMenuFragment.DM_INDEX) {
    ...
    protected void buildListAdapter() {
        timeLineAdapter = new DMUserListAdapter(this, getList().getItemList(),
                getListView());
        getListView().setAdapter(timeLineAdapter);
    }
    
    private class DBCacheTask extends
            MyAsyncTask<Void, DMUserListBean, DMUserListBean> {
        ...
        protected DMUserListBean doInBackground(Void... params) {
            return DMDBTask.get(GlobalContext.getInstance()
                    .getCurrentAccountId());
        }
    ...
    @Override
    protected void newMsgLoaderSuccessCallback(DMUserListBean newValue,
            Bundle loaderArgs) {
        if (newValue != null && newValue.getSize() > 0 && getActivity() != null) {
            getList().addNewData(newValue);
            getAdapter().notifyDataSetChanged();
            getListView().setSelectionAfterHeaderView();
            DMDBTask.asyncReplace(getList(), GlobalContext.getInstance()
                    .getCurrentAccountId());
        }
    }
    ...
    protected Loader<AsyncTaskLoaderResult<DMUserListBean>> onCreateNewMsgLoader(
            int id, Bundle args) {
    ...
    protected Loader<AsyncTaskLoaderResult<DMUserListBean>> onCreateOldMsgLoader(
            int id, Bundle args) {
        String token = GlobalContext.getInstance().getSpecialToken();
        String cursor = null;
        if (getList().getSize() > 0
