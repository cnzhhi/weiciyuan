package me.ivps.android.myweibo.othercomponent.sendweiboservice;
...
import me.ivps.android.myweibo.dao.send.StatusNewMsgDao;
...
import me.ivps.android.myweibo.support.utils.NotificationUtility;
...
public class SendWeiboService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        StatusDraftBean statusDraftBean = (StatusDraftBean) intent
                .getParcelableExtra("draft");
        
        WeiboSendTask task = new WeiboSendTask(lastNotificationId, token,
                account, picPath, content, geoBean, statusDraftBean);
    ...
    private class WeiboSendTask extends MyAsyncTask<Void, Long, Void> {
        ...
        public WeiboSendTask(int lastNotificationId, String token,
                AccountBean account, String picPath, String content,
        ...
        protected void onPreExecute() {
            ...
            Notification.Builder builder = new Notification.Builder(
                    ...
                    .setOngoing(true).setSmallIcon(R.drawable.upload_white);
            ...
            if (Utility.isJB()) {
                ...
                IntentFilter intentFilter = new IntentFilter(
                        "org.qii.weiciyuan.SendWeiboService.stop."
                ...
                pendingIntent = PendingIntent.getBroadcast(
                        SendWeiboService.this, 1, broadcastIntent,
                        PendingIntent.FLAG_UPDATE_CURRENT);
                builder.addAction(R.drawable.send_failed,
                        getString(R.string.cancel), pendingIntent);
        ...
        private boolean sendPic(String uploadPicPath) throws WeiboException {
            return new StatusNewMsgDao(token)
                    .setPic(uploadPicPath)
                    .setGeoBean(geoBean)
                    .sendNewMsg(content,
                            new FileUploaderHttpHelper.ProgressListener() {
        ...
        private boolean sendText() throws WeiboException {
            return new StatusNewMsgDao(token).setGeoBean(geoBean).sendNewMsg(
                    content, null);
        }
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                if (!TextUtils.isEmpty(picPath)) {
                    String uploadPicPath = ImageUtility.compressPic(
                            SendWeiboService.this, picPath);
            ...
            if (values.length > 0) {
                ...
                if (data != -1) {
                    ...
                    Notification.Builder builder = new Notification.Builder(
                            ...
                            .setContentTitle(
                                    getString(R.string.background_sending))
                            .setNumber((int) (r * 100)).setContentText(content)
                    ...
                    Notification.Builder builder = new Notification.Builder(
                            ...
                            .setTicker(getString(R.string.send_photo))
                            .setContentTitle(
                                    getString(R.string.wait_server_response))
                            .setContentText(content).setNumber(100)
                ...
                NotificationUtility.show(notification,
                        tasksNotifications.get(WeiboSendTask.this));
            }
        }
        ...
        private void showFailedNotification(final WeiboSendTask task) {
            Notification.Builder builder = new Notification.Builder(
                    ...
                    .setContentText(content).setOnlyAlertOnce(true)
                    .setAutoCancel(true).setSmallIcon(R.drawable.send_failed)
                    .setOngoing(false);
            
            Intent notifyIntent = WriteWeiboActivity.startBecauseSendFailed(
                    SendWeiboService.this, account, content, picPath, geoBean,
                    statusDraftBean, String.format(SendWeiboService.this
                            .getString(R.string.failed_reason), e.getError()));
            
            PendingIntent pendingIntent = PendingIntent.getActivity(
                    SendWeiboService.this, 0, notifyIntent,
                    PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            if (Utility.isJB()) {
                if (TextUtils.isEmpty(picPath)) {
                    ...
                    Bitmap bitmap = ImageUtility
                            .getNotificationSendFailedPic(picPath);
                    if (bitmap != null) {
                ...
                Intent intent = new Intent(SendWeiboService.this,
                ...
                PendingIntent retrySendIntent = PendingIntent.getService(
                        SendWeiboService.this, 0, intent,
                        PendingIntent.FLAG_UPDATE_CURRENT);
