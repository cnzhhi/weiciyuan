...
import me.ivps.android.myweibo.dao.send.CommentNewMsgDao;
...
public class SendCommentService extends Service {
    ...
    public static Intent newIntent(AccountBean accountBean, MessageBean msg,
            String content, boolean comment_ori) {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        CommentDraftBean commentDraftBean = (CommentDraftBean) intent
                .getParcelableExtra("draft");
        
        WeiboSendTask task = new WeiboSendTask(account, token, content, oriMsg,
                comment_ori, commentDraftBean);
    ...
    private class WeiboSendTask extends MyAsyncTask<Void, Long, Void> {
        ...
        public WeiboSendTask(AccountBean account, String token, String content,
                MessageBean oriMsg, boolean comment_ori,
                CommentDraftBean commentDraftBean) {
        ...
        private CommentBean sendText() throws WeiboException {
            CommentNewMsgDao dao = new CommentNewMsgDao(token, oriMsg.getId(),
                    content);
        ...
        private void showSuccessfulNotification(final WeiboSendTask task) {
            ...
            LocalBroadcastManager
                    .getInstance(SendCommentService.this)
                    .sendBroadcast(
                            new Intent(
                                    AppEventAction
        ...
        private void showFailedNotification(final WeiboSendTask task) {
            ...
            Intent notifyIntent = WriteCommentActivity.startBecauseSendFailed(
                    SendCommentService.this, account, content, oriMsg,
                    commentDraftBean, comment_ori, e.getError());
            
            PendingIntent pendingIntent = PendingIntent.getActivity(
                    SendCommentService.this, 0, notifyIntent,
            ...
            if (Utility.isJB()) {
                ...
                PendingIntent retrySendIntent = PendingIntent.getService(
                        SendCommentService.this, 0, intent,
                        PendingIntent.FLAG_UPDATE_CURRENT);
