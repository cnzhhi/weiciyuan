...
import me.ivps.android.myweibo.bean.ItemBean;
...
import me.ivps.android.myweibo.bean.android.UnreadTabIndex;
...
import me.ivps.android.myweibo.ui.send.WriteCommentActivity;
import me.ivps.android.myweibo.ui.send.WriteReplyToCommentActivity;
...
public class BigTextNotificationService extends NotificationServiceHelper {
    ...
    public static Intent newIntent(AccountBean accountBean,
            ...
            Intent clickNotificationToOpenAppPendingIntentInner, String ticker,
            int currentIndex) {
        ...
        intent.putExtra(NotificationServiceHelper.MENTIONS_COMMENT_ARG,
                mentionsCommentData);
        intent.putExtra(NotificationServiceHelper.COMMENTS_TO_ME_ARG,
                commentsToMeData);
        intent.putExtra(NotificationServiceHelper.UNREAD_ARG, unreadBean);
        intent.putExtra(NotificationServiceHelper.CURRENT_INDEX_ARG,
                currentIndex);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        AccountBean accountBean = intent
                .getParcelableExtra(NotificationServiceHelper.ACCOUNT_ARG);
        ...
        valueWrapper.accountBean = intent
                .getParcelableExtra(NotificationServiceHelper.ACCOUNT_ARG);
        ...
        valueWrapper.unreadBean = intent
                .getParcelableExtra(NotificationServiceHelper.UNREAD_ARG);
        valueWrapper.currentIndex = intent.getIntExtra(
                NotificationServiceHelper.CURRENT_INDEX_ARG, 0);
        ...
        valueWrapper.ticker = intent
                .getStringExtra(NotificationServiceHelper.TICKER);
        ...
        if (valueWrapper.mentionsComment != null) {
            notificationItems
                    .addAll(valueWrapper.mentionsComment.getItemList());
        }
        ...
        AppLogger.i("service account name="
                + valueWrapper.accountBean.getUsernick());
    ...
    private void buildNotification(String uid) {
        ...
        Notification.Builder builder = new Notification.Builder(
                ...
                .setAutoCancel(true)
                .setContentIntent(
                        getPendingIntent(clickToOpenAppPendingIntentInner,
                                itemBean, accountBean)).setOnlyAlertOnce(true);
                    ...
                    public void run() {
                        try {
                            ...
                            if (mentionsWeibo != null) {
                                for (MessageBean msg : mentionsWeibo
                                        .getItemList()) {
                                    ids.add(msg.getId());
                                }
                            ...
                            if (commentsToMe != null) {
                                ...
                                for (CommentBean msg : commentsToMe
                                        .getItemList()) {
                                    ids.add(msg.getId());
                                }
                            ...
                            if (mentionsComment != null) {
                                for (CommentBean msg : mentionsComment
                                        .getItemList()) {
                                    ids.add(msg.getId());
                                }
        ...
        IntentFilter intentFilter = new IntentFilter(
                RESET_UNREAD_MENTIONS_WEIBO_ACTION);
        
        Utility.registerReceiverIgnoredReceiverHasRegisteredHereException(
                GlobalContext.getInstance(),
                valueWrapper.clearNotificationEventReceiver, intentFilter);
        ...
        PendingIntent deletedPendingIntent = PendingIntent.getBroadcast(
                GlobalContext.getInstance(), accountBean.getUid().hashCode(),
                broadcastIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        builder.setDeleteIntent(deletedPendingIntent);
        
        if (itemBean instanceof MessageBean) {
            MessageBean msg = (MessageBean) itemBean;
            Intent intent = WriteCommentActivity.newIntentFromNotification(
                    getApplicationContext(), accountBean, msg);
            PendingIntent pendingIntent = PendingIntent.getActivity(
                    getApplicationContext(), 0, intent,
                    PendingIntent.FLAG_UPDATE_CURRENT);
            builder.addAction(R.drawable.comment_light, getApplicationContext()
                    .getString(R.string.comments), pendingIntent);
        }
            ...
            Intent intent = WriteReplyToCommentActivity
                    .newIntentFromNotification(getApplicationContext(),
                            accountBean, commentBean);
            PendingIntent pendingIntent = PendingIntent.getActivity(
                    getApplicationContext(), 0, intent,
                    PendingIntent.FLAG_UPDATE_CURRENT);
            builder.addAction(R.drawable.reply_to_comment_light,
                    getApplicationContext()
                            .getString(R.string.reply_to_comment),
                    pendingIntent);
        }
        
        String avatar = ((ItemBean) itemBean).getUser().getAvatar_large();
        String avatarPath = FileManager.getFilePathFromUrl(avatar,
                FileLocationMethod.avatar_large);
        if (ImageUtility.isThisBitmapCanRead(avatarPath)
                && TaskCache.isThisUrlTaskFinished(avatar)) {
        ...
        if (count > 1) {
            ...
            Intent nextIntent = BigTextNotificationService.newIntent(
                    accountBean, mentionsWeibo, commentsToMe, mentionsComment,
                    unreadBean, clickToOpenAppPendingIntentInner, ticker,
                    nextIndex);
            PendingIntent retrySendIntent = PendingIntent.getService(
                    BigTextNotificationService.this, accountBean.getUid()
                            .hashCode(), nextIntent,
                    PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        Notification.BigTextStyle bigTextStyle = new Notification.BigTextStyle(
                builder);
        bigTextStyle.setBigContentTitle(getItemBigContentTitle(accountBean,
                notificationItems, currentIndex));
        bigTextStyle.bigText(getItemBigText(notificationItems, currentIndex));
        String summaryText;
        if (count > 1) {
            summaryText = accountBean.getUsernick() + "(" + (currentIndex + 1)
                    + "/" + count + ")";
        }
        else {
        ...
        notificationManager.notify(getMentionsWeiboNotificationId(accountBean),
                builder.build());
    }
    
    private PendingIntent getPendingIntent(
            Intent clickToOpenAppPendingIntentInner, Parcelable itemBean,
            AccountBean accountBean) {
        clickToOpenAppPendingIntentInner.setExtrasClassLoader(getClass()
                .getClassLoader());
        ...
        clickToOpenAppPendingIntentInner
                .putExtra(BundleArgsConstants.OPEN_NAVIGATION_INDEX_EXTRA,
                        unreadTabIndex);
        PendingIntent pendingIntent = PendingIntent.getActivity(
                getBaseContext(), getMentionsWeiboNotificationId(accountBean),
                clickToOpenAppPendingIntentInner,
                PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    private CharSequence getItemBigText(
            ArrayList<Parcelable> notificationItems, int currentIndex) {
        ItemBean itemBean = (ItemBean) notificationItems.get(currentIndex);
        return TimeLineUtility.convertNormalStringToSpannableString(itemBean
                .getText());
    }
}
