...
import me.ivps.android.myweibo.othercomponent.unreadnotification.BigTextNotificationService;
...
public class AppNotificationCenter {
    ...
    private HashMap<AccountBean, MessageListBean> unreadMentions = new HashMap<AccountBean, MessageListBean>();
    private HashMap<AccountBean, CommentListBean> unreadMentionsComment = new HashMap<AccountBean, CommentListBean>();
    private HashMap<AccountBean, CommentListBean> unreadComments = new HashMap<AccountBean, CommentListBean>();
    ...
    public void addUnreadMentions(final AccountBean account,
            final MessageListBean data) {
        this.unreadMentions.put(account, data);
    }
    
    public void addUnreadMentionsComment(final AccountBean account,
            final CommentListBean data) {
        this.unreadMentionsComment.put(account, data);
    }
    
    public void addUnreadComments(final AccountBean account,
            final CommentListBean data) {
    ...
    public void refreshToUI(final AccountBean account) {
        ...
        final CommentListBean mentionsComment = unreadMentionsComment
                .get(account);
        ...
        this.uiHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                for (Callback callback : callbackList) {
                    if (mentions != null) {
                        AppLogger.i("Post unread mention weibo data to ui: "
                                + callback.getClass().getSimpleName());
                        callback.unreadMentionsChanged(account, mentions);
                    }
                    if (comments != null) {
                        AppLogger.i("Post unread comment to me data to ui: "
                                + callback.getClass().getSimpleName());
                        callback.unreadCommentsChanged(account, comments);
                    }
                    if (mentionsComment != null) {
                        AppLogger.i("Post unread mention comments data to ui: "
                                + callback.getClass().getSimpleName());
                        callback.unreadMentionsCommentChanged(account,
                                mentionsComment);
                    }
                }
            }
    ...
    public void showAndroidNotification(AccountBean account) {
        ...
        showNotification(GlobalContext.getInstance(), account, mentions,
                comments, mentionsComment, unreadBean);
    }
    
    private void addDatabaseUnreadMentionsWeibo(String accountId,
            MessageListBean mentions) {
    ...
    private void addDatabaseUnreadMentionsComment(String accountId,
            CommentListBean mentions) {
    ...
    private void addDatabaseUnreadCommentsToMe(String accountId,
            CommentListBean mentions) {
