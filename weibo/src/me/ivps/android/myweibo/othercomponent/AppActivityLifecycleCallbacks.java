...
public class AppActivityLifecycleCallbacks implements
    ...
    public void onActivityCreated(Activity activity, Bundle savedInstanceState) {
        AppLogger.i("Activity is created, class name : "
                + activity.getClass().getSimpleName());
    }
    
    @Override
    public void onActivityStarted(Activity activity) {
        AppLogger.i("Activity is started, class name : "
                + activity.getClass().getSimpleName());
    ...
    public void onActivityStopped(Activity activity) {
        AppLogger.i("Activity is stopped, class name : "
                + activity.getClass().getSimpleName());
        visibleActivityCount--;
        if (visibleActivityCount == 0) {
            stopFetchUnread();
    ...
    public void onActivitySaveInstanceState(Activity activity, Bundle outState) {
        AppLogger.i("Activity saved instance state, class name : "
                + activity.getClass().getSimpleName());
    }
    
    @Override
    public void onActivityDestroyed(Activity activity) {
        AppLogger.i("Activity is destroyed, class name : "
                + activity.getClass().getSimpleName());
    }
    
    private void startFetchUnread() {
        uiHandler
                .postDelayed(scheduleFetchUnreadRunnable, SCHEDULE_DELAY_MILLS);
        uiHandler.removeCallbacks(unScheduleFetchUnreadRunnable);
    }
    
    private void stopFetchUnread() {
        uiHandler.postDelayed(unScheduleFetchUnreadRunnable,
    ...
    private Runnable fetchRunnable = new Runnable() {
        ...
        public void run() {
            ...
            GlobalContext.getInstance().startService(
                    FetchNewMsgService.newIntentFromOpenApp());
        }
    };
}
